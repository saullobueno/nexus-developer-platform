...
> **Status atual: Phase 17 — Security hardening.** Foundation, Database, Auth/RBAC, Demo Mode, Home, Service Catalog, Deployments, Incidents, Observability, APIs/Docs, Feature Flags, Pipelines, Teams/Reports, Integrations, Realtime, AI Copilot e Settings/Audit completos. Esta fase adicionou rate limiting (`@nestjs/throttler`, global + limite restrito em `/auth/login` contra brute-force), security headers (`helmet`) e uma nova aba **Webhooks** em `/settings`, onde o segredo gerado na criação é mostrado uma única vez e persistido apenas como hash bcrypt (reaproveitando `hashPassword`/`verifyPassword` de `@nexus/auth`) — a primeira feature real sobre a tabela `webhooks`, existente desde a Phase 1. Cookies (`httpOnly`/`secure`/`sameSite`), RBAC e object-level authorization já estavam sólidos desde fases anteriores. Ver ADR `docs/decisions/0016-security-hardening-rate-limiting-headers-webhooks.md` para o detalhamento e os gaps documentados (2FA, SSO, criptografia em repouso). Ver `NEXUS-SPECIFICATION.md` para a especificação completa e o roadmap de fases.
