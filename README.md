...
> **Status atual: Phase 15 — AI Engineering Copilot.** Foundation, Database, Auth/RBAC, Demo Mode, Home, Service Catalog, Deployments, Incidents, Observability, APIs/Docs, Feature Flags, Pipelines, Teams/Reports, Integrations e Realtime completos. `/ai-copilot` responde perguntas como "Why is payments-api slow today?" usando 12 ferramentas somente-leitura reais (get_service, get_service_metrics, search_logs, get_incident etc.) sobre os mesmos dados do resto do produto — nunca inventa evidência. Ações mutáveis (rollback_deployment, create_incident, update_feature_flag, trigger_deployment) só são sugeridas, nunca executadas automaticamente: exigem aprovação humana explícita. Usa o Vercel AI SDK (`generateText` + tool calling) com Anthropic/OpenAI quando configurado; sem credenciais (ou em `DEMO_MODE=true`), cai para um orchestrator determinístico que exercita as mesmas ferramentas reais. Toda execução é auditada (`ai_agent_runs`/`ai_messages`/`ai_tool_calls`). Ver `NEXUS-SPECIFICATION.md` para a especificação completa e o roadmap de fases.
