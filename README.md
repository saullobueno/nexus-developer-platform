...
> **Status atual: Phase 16 — Settings/Audit.** Foundation, Database, Auth/RBAC, Demo Mode, Home, Service Catalog, Deployments, Incidents, Observability, APIs/Docs, Feature Flags, Pipelines, Teams/Reports, Integrations, Realtime e AI Copilot completos. `/settings` ganhou abas Organization (nome/timezone editáveis), Members (troca de role por membro), Roles (as 5 roles baseline com suas permissions, somente leitura), Environments (nome/URL/tipo) e Audit Logs (todo o histórico de `audit_logs`, já escrito desde a Phase 6, agora com uma UI filtrável e paginada) — além de Integrations (Phase 13). Notifications e Security (SSO/2FA/API keys) ficam documentadas como gaps conhecidos: não há infraestrutura real por trás delas ainda, e o projeto evita construir telas que não fazem nada de verdade. Ver `NEXUS-SPECIFICATION.md` para a especificação completa e o roadmap de fases.
