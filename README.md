...
> **Status atual: Phase 14 — Realtime.** Foundation, Database, Auth/RBAC, Demo Mode, Home, Service Catalog, Deployments, Incidents, Observability, APIs/Docs, Feature Flags, Pipelines, Teams/Reports e Integrations completos. `GET /realtime/events` expõe um stream SSE (Server-Sent Events, sem WebSocket/dependência nova) com os eventos tipados `deployment.started/updated/completed` e `incident.created/updated/resolved`, sempre escopados por `organizationId`. No frontend, `useRealtimeEvents` mantém a conexão viva, invalida as queries do TanStack Query automaticamente e alimenta um toaster (`RealtimeToaster`) que mostra as atualizações em tempo real — abra `/deployments` em duas abas e cancele um deployment numa delas para ver a outra atualizar sozinha. Ver `NEXUS-SPECIFICATION.md` para a especificação completa e o roadmap de fases.
