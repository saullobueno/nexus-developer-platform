...
> **Status atual: Phase 13 — Integrations.** Foundation, Database, Auth/RBAC, Demo Mode, Home, Service Catalog, Deployments, Incidents, Observability, APIs/Docs, Feature Flags, Pipelines e Teams/Reports completos. `@nexus/integrations` ganhou adapters reais (GitHubAdapter, SentryAdapter, GrafanaAdapter, SlackAdapter) além do `MockAdapter` da Phase 3 — todos implementando a mesma `IntegrationProvider`. `resolveAdapter` escolhe o adapter real ou cai para `MockAdapter` quando `DEMO_MODE=true` ou faltam credenciais, nunca quebrando o demo. `/settings` traz a seção Integrations (GitHub/Sentry/Grafana/Slack/Prometheus) com config editável, toggle enabled e "Testar conexão". Ver `NEXUS-SPECIFICATION.md` para a especificação completa e o roadmap de fases.
