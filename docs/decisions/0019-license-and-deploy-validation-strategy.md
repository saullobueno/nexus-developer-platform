...
## Addendum 4 — stack de deploy final escolhida e confirmada funcionando

Entre as opções levantadas (comparadas direto nas páginas oficiais de cada provedor, não em agregadores de blog genéricos — free tier muda com frequência e vira desinformação rápido), a escolha final, **sem custo**:

- **Postgres**: [Neon](https://neon.com) — plano Free permanente (não expira como o do Render, que expira em 30 dias; não pausa por inatividade toda semana como o do Supabase), sem cartão de crédito, 0,5 GB de storage/100 CU-hours por mês — suficiente para o dataset de demo do Nexus. O driver `postgres` usado em `packages/database/src/client.ts` já lê `sslmode=require` direto da connection string, sem precisar de nenhuma mudança de código.
- **`apps/api`**: [Render](https://render.com) — free tier real sem cartão de crédito para um Web Service Node contínuo (750h/mês). Trade-off aceito: o serviço "dorme" depois de 15 min sem tráfego, com ~30-60s de cold start no primeiro request depois disso (aceitável para portfólio, não seria para produção real). Fly.io e Railway foram descartados por exigirem cartão de crédito ou terem um free tier de fato inviável (ver comparação completa na conversa que gerou este addendum).
- **`apps/web`**: Vercel (Hobby, já decidido nos addendums anteriores).

Fluxo de setup executado: criar projeto no Neon → copiar a connection string → rodar `pnpm --filter @nexus/database db:migrate` e `db:seed:demo` localmente apontando `DATABASE_URL` pra essa string (via `$env:DATABASE_URL` no PowerShell) → criar o Web Service no Render com Root Directory `apps/api`, Build Command reconstruindo o workspace (`cd ../.. && pnpm install --frozen-lockfile && pnpm turbo run build --filter=api && cd apps/api`) e Start Command `node dist/main.js` → configurar `NEXT_PUBLIC_API_URL` na Vercel apontando pro Render e redeployar `apps/web`.

**Confirmado funcionando de ponta a ponta**: login via `admin@acme.test`/`demo1234` autentica corretamente contra a API no Render e o Postgres no Neon, com o cookie `sameSite=none` (Addendum 3) sendo aceito pelo browser entre os dois domínios diferentes.

Nota de segurança registrada aqui para o histórico: a senha inicial da role do Neon foi colada em texto puro numa conversa de terminal durante o setup — foi rotacionada imediatamente depois (reset de password no console do Neon) antes de qualquer uso em produção.
