...
- A tabela `documents` ganhou `serviceId` (nullable) na Phase 9 — a tab "Documentation" do Service Detail e da API Detail agora mostram documentos vinculados de fato. Documentos sem `serviceId` (ex.: Getting Started, Padrões de Engenharia) continuam existindo como conteúdo só acessível via `/docs`, o que é esperado (nem todo documento é específico de um serviço).
